...
function metadata_show() {
    ...
    div += "<div class=\"hbi_tab\">" + trans_value["metadata"]["name"] + " <img src=\"images/reload.png\" border=\"0\" onClick=\"load_init()\"/></div>";
    div += "</div>";
    div += "<div>";
    for (var i = 0; i < object_metadata_list.length; i++) {
...
function metadata_print_relation(metadata) {
    ...
    for(var i=0;i<metadata.metadata_relation_list.length;i++) {
        ...
        var type = "";
        if(mr.type*1===1) { type = trans_value["metadata"]["relation"]["type"]["1_N"]; }
        if(mr.type*1===2) { type = trans_value["metadata"]["relation"]["type"]["N_1"]; }
        if(mr.type*1===3) { type = trans_value["metadata"]["relation"]["type"]["N_M"]; }
